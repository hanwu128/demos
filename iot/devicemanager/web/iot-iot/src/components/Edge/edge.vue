  ...
  <div class="body_content">
    <div class="menu_list">
      <ul class="operate_nav">
                     ...
                     <el-collapse-transition>
                         <div class="operate_dd" v-if="menu_strong_show[0]">
                             <dl>
                                 ...
                                 <dd @click="menuClick('dd3')" class="dd3" v-if='permission_list[3]'><router-link to="/edge/deImage"><em></em>设备镜像管理</router-link></dd>
                                 <dd @click="menuClick('dd4')" class="dd4" v-if='permission_list[4]'><router-link to="/edge/processModule"><em></em>处理模块管理</router-link></dd>
                                 &lt;!&ndash; <dd @click="menuClick('dd5')" class="dd5" :class="{'active':menu_dd_active[4]}"><router-link to="/edge/labelManager"><em></em>标签管理</router-link></dd> &ndash;&gt;
                             </dl>
                         </div>
                     </el-collapse-transition>
                 </li>-->
        ...
        <!--2018.3.7新增链接-->
        <li class="li3">
          <router-link to="/edge/manager"><label class="operate_nav_a" :class="{'active':module_name=='manager'}" @click="menu_li_click(5,'manager')"><i></i><span>设备管理</span></label></router-link>
        </li>
        <li class="li5">
          <router-link to="/edge/deljs"><label class="operate_nav_a" :class="{'active':module_name=='deljs'}" @click="menu_li_click(4,'deljs')"><i></i><span>流计算</span></label></router-link>
        </li>
        <li class="li4">
          <router-link to="/edge/deImage"><label class="operate_nav_a" :class="{'active':module_name=='deImage'}" @click="menu_li_click(3,'deImage')"><i></i><span>数字镜像</span></label></router-link>
        </li>
        <!-- <li class="li5">
        ...
        <li class="li5">
          <router-link to="/edge/desjdc"><label class="operate_nav_a" :class="{'active':module_name=='desjdc'}" @click="menu_li_click(4,'desjdc')"><i></i><span>数据洞察</span></label></router-link>
        </li>
...
<script>
  export default {
    ...
    data () {
      return {
        menu_li_show:[false,false,false,false,false,true],
    ...
    methods:{
      ...
      menu_li_click:function(status,type){
        if(type==this.module_name){
          history.go(0);
        }
        if(status == 0){
          this.menu_strong_show = [!this.menu_strong_show[status],false,false];
          this.set_menu(0);
        }else if(status == 1){
          this.menu_strong_show = [false,!this.menu_strong_show[status],false,false,false,false];
          this.set_menu(0);
        }else if(status == 2){
          this.menu_strong_show = [false,false,!this.menu_strong_show[status],false,false,false];
          this.set_menu(0);
        }else if(status == 3){
          this.menu_strong_show = [false,false,false,true,false,false];
          this.set_menu(1);
        }else if(status == 4){
          this.menu_strong_show = [false,false,false,false,!this.menu_strong_show[status],false];
          this.set_menu(0);
        }else if(status == 5){
          this.menu_strong_show = [false,false,false,false,false,!this.menu_strong_show[status]];
          this.set_menu(0);
        }
      },
      ...
      set_menu:function(status){
        ...
        if(module_name == 'edge' || module_name == 'manager' || module_name == 'detail' || module_name == 'insert'){
          if(status == 1){
            this.menu_li_show=[true,false,false,false,false,false];
    ...
    created: function(){
      let page_path = window.location.hash;
      this.module_name = page_path.split('/').pop().split('?')[0];
      this.permissionFn();
      this.set_menu(1);
