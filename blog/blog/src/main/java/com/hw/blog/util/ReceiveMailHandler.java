...
public class ReceiveMailHandler {
    ...
    private static ReceiveMailHandler receiveMailHandler;

    @PostConstruct
    public void init() {
        receiveMailHandler = this;
    }
    ...
    private static Session getSessionMail() throws Exception {
        ...
        properties.put("mail.smtp.host", receiveMailHandler.config.getMAIL_POP3_HOST());
        properties.put("mail.smtp.auth", receiveMailHandler.config.getMAIL_POP3_AUTH());
    ...
    public static void receiveMail() {
        ...
        try {
            //进行用户邮箱连接
            urln = new URLName(receiveMailHandler.config.getMAIL_POP3_TYPE(), receiveMailHandler.config.getMAIL_POP3_HOST(), receiveMailHandler.config.getMAIL_POP3_PORT(), null, receiveMailHandler.config.getMAIL_USER(), receiveMailHandler.config.getMAIL_PWD());
            ...
            if (messageCount > 0) {
                ...
                for (int i = 0; i < messages.length; i++) {
                    ...
                    if (isContainAttach((Part) messages[i])) {
                        saveAttachMent((Part) messages[i], receiveMailHandler.config.getMAIL_ATTACH_PATH());
                    }
