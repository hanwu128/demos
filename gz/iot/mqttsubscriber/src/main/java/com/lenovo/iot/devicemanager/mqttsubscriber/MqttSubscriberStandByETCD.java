...
public class MqttSubscriberStandByETCD implements MqttCallback {
	...
	//for etcd init
	public MqttSubscriberStandByETCD(String topicFromETCDBFC, String deviceidFromETCDBFC, String access_key, String secret_key)throws Exception{

		String configFile = "mqtt.conf";

		String broker = "ssl://223.203.218.93:8883";
		String clientId = deviceidFromETCDBFC;
		String userName = access_key;
		String password = secret_key;
		String topic = topicFromETCDBFC;
		int qos = 1;
		int keepAlive = 90;
		...
		System.out.println("clientId_MqttSubscriberStandByETCD:"+clientId);
		System.out.println("userName_MqttSubscriberStandByETCD:"+userName);
		System.out.println("password_MqttSubscriberStandByETCD:"+password);


		// Read settings from configuration file
		try {
			String path = MqttSubscriberStandByETCD.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
		...
		try {
			...
			MqttSubscriberStandByETCD emqtt = new MqttSubscriberStandByETCD(broker, clientId, cleanSession, userName, password, keepAlive);
	...
	public MqttSubscriberStandByETCD(String brokerUrl, String clientId, boolean cleanSession, String userName, String password, int keepAlive) throws MqttException {
...
//            }
	...
	public void messageArrived(String topic, MqttMessage message) throws MqttException {
		...
		try {
			if(topic.startsWith("$SYS/brokers/")) {
			...
			} else if(topic.startsWith("$MOC/device/")) {
				...
				if(topicArray[3].equals("shadow")) {
					...
					if(topicArray[4].equals("update")) {
						...
						sb.append("ApachEdgentVersionController/insertApacheAdgentMetaData.url");
						HttpClient.httpPost(sb.toString(), JSONObjectmessageString);
					}
					...
					if(topicArray[4].equals("update")) {
						if(topicArray.length>4 && topicArray[5].equals("reponse")) {
							...
							String is_success = JSONObject_messageString.getString("is_success");

							//设备上报基本信息
							JSONObject _JSONObject4Message = new JSONObject();
							_JSONObject4Message.put("task_type","task_type");
							_JSONObject4Message.put("taskId",task_id);
							_JSONObject4Message.put("statusCallbackFromApacheAdgetn",is_success);
							//_JSONObject4Message.put("taskid","")

							StringBuffer sb = new StringBuffer();
							sb.append("ApachEdgentVersionController/checkApacheAdgentBroker.url");
							HttpClient.httpPost(sb.toString(), _JSONObject4Message);
						}
					}
				}
			} else {
